...
HTML = ROOT / "推免套磁/华中师范大学/华中师范大学计算机学院教师名录及研究方向.html"
TEMPLATE = ROOT / "推免套磁/套磁信-大模型工程方向-修改版.md"
OUT = ROOT / "推免套磁/华中师范大学/个性化套磁信"
...
def paragraphs(direction):
    ...
    if any(k in d for k in ("大模型", "自然语言", "中文信息", "知识", "问答", "信息检索", "推荐")):
        return (
            f"您主要从事{d}等方面的研究，这与我希望继续深耕的大模型应用、"
            "自然语言处理和 Agent 工程方向高度契合。",
            ...
            f"若有机会加入您的课题组，我希望围绕{d}深入学习文本表示、信息抽取、知识增强、"
            "大模型推理与 Agent 评测，将已有的工程实践提升为规范的科研能力。",
        )
    if any(k in d for k in ("视觉", "图像", "视频", "多模态", "模式识别", "图形")):
        return (
    ...
    if any(k in d for k in ("安全", "隐私", "密码", "攻防", "联邦", "区块链")):
        return (
    ...
    if any(k in d for k in ("边缘", "分布式", "物联网", "云计算", "智能网络", "网络", "移动")):
        return (
    ...
    if any(k in d for k in ("机器学习", "深度学习", "数据挖掘", "优化", "强化学习", "人工智能")):
        return (
...
if "<th>套磁信</th>" not in text:
    text = text.replace(
        '<th data-page-node-id="XfImQYPTmLw5PP1Sa2w5hU">主要研究方向</th>',
        '<th data-page-node-id="XfImQYPTmLw5PP1Sa2w5hU">主要研究方向</th><th>套磁信</th>',
    )
