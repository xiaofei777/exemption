...
HTML = ROOT / "推免套磁/西南交通大学/西南交通大学计算机学院教师名录及研究方向.html"
TEMPLATE = ROOT / "推免套磁/套磁信-大模型工程方向-修改版.md"
OUT = ROOT / "推免套磁/西南交通大学/个性化套磁信"
...
def paragraphs(direction):
    ...
    if any(k in d for k in ("Agent", "智能体", "大模型", "生成式", "自然语言", "知识图谱", "推荐")):
        return (
    ...
    if any(k in d for k in ("视觉", "图像", "视频", "模式识别", "多模态", "高光谱", "计算成像", "目标")):
        return (
    ...
    if any(k in d for k in ("安全", "密码", "隐私", "网络攻防", "可信", "漏洞")):
        return (
    ...
    if any(k in d for k in ("边缘", "物联网", "云计算", "分布式", "网络", "智能交通", "车联网", "嵌入式")):
        return (
    ...
    if any(k in d for k in ("软件工程", "软件测试", "程序理解", "人机交互", "服务计算", "系统建模")):
        return (
    ...
    if any(k in d for k in ("机器学习", "深度学习", "数据挖掘", "人工智能", "强化学习", "优化", "数据分析")):
        return (
...
def make_letter(row):
    index, name_cell, title, advisor, age, email, direction, homepage = row
    name = clean_name(name_cell)
    direction_display = clean_direction(direction)
    email_display = EMAIL_OVERRIDES.get(name, email)
    if email_display in {"未公开", "—", ""}:
        email_display = "官网未公开，请以教师主页为准"
    subject = (
...
if "<th>套磁信</th>" not in text:
    text = text.replace(
        '<th data-page-node-id="jboaC8GdJ6oC3rdOlRPoWI">主要研究方向</th>',
        '<th data-page-node-id="jboaC8GdJ6oC3rdOlRPoWI">主要研究方向</th><th>套磁信</th>',
    )
if ".letter-cell" not in text:
    text = text.replace(
        " .dr{color:#374151;line-height:1.7;overflow-wrap:anywhere;word-break:break-word;min-width:240px}\n",
        " .dr{color:#374151;line-height:1.7;overflow-wrap:anywhere;word-break:break-word;min-width:240px}\n"
