...
HTML = ROOT / "推免套磁/上海大学/上海大学计算机学院教师名录及研究方向.html"
TEMPLATE = ROOT / "推免套磁/套磁信-大模型工程方向-修改版.md"
OUT = ROOT / "推免套磁/上海大学/个性化套磁信"
...
class FacultyParser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        if tag == "td" and self.in_td:
            ...
            if len(self.row) >= 7 and self.row[0].isdigit():
                self.rows.append(self.row[:7] + [self.href])
            self.in_tr = False
            self.href = ""
...
def clean_direction(direction):
    ...
    direction = re.sub(r"（.*?地点：.*?$", "", direction).strip()
    direction = re.sub(r"\s+", " ", direction)
    if direction.startswith("未公开"):
        return "人工智能相关研究"
    return direction or "人工智能相关研究"


def customized_paragraphs(direction):
    original = direction
    d = clean_direction(direction)
    if original.startswith("未公开"):
        return (
    ...
    if any(k in d for k in ("大模型", "大语言模型", "自然语言", "智能体", "知识图谱", "知识工程", "推荐")):
        return (
    ...
    if any(k in d for k in ("视觉", "图像", "视频", "模式识别", "多模态", "图形", "AIGC", "生成式")):
        return (
    ...
    if any(k in d for k in ("安全", "隐私", "密码", "可信", "网络空间", "区块链")):
        return (
    ...
    if any(k in d for k in ("机器人", "具身", "智能决策", "多智能体", "强化学习", "导航", "车联网")):
        return (
            f"您主要从事{d}等方面的研究，研究问题兼具智能感知、任务决策和复杂工程约束，"
            "与我对智能体和 AI 系统落地的兴趣有较强交叉。",
            "我主导过面向军工场景的 CamTalk 多模态实时视觉对话 Agent，完成端侧数据最小化、"
            "关键帧智能采样和 STT–LLM–TTS 流式编排，关注实时延迟、断线恢复和复杂环境下的系统可靠性。",
            f"若有机会加入您的课题组，我希望围绕{d}学习感知决策、任务规划、强化学习和智能体协同方法，"
            "探索大模型 Agent 在真实装备和复杂环境中的应用。",
        )
    if any(k in d for k in ("边缘", "联邦", "分布式", "云计算", "移动计算", "物联网", "计算机网络", "算力")):
        return (
            f"您主要从事{d}等方面的研究，与我对端边云协同、实时智能系统和 AI 工程落地的兴趣有较好交叉。",
    ...
    if any(k in d for k in ("机器学习", "深度学习", "数据挖掘", "数据分析", "智能计算", "优化", "科学计算")):
        return (
...
def make_letter(row):
    index, name_cell, category, title, age, email, direction, homepage = row
    name = clean_name(name_cell)
    direction_display = clean_direction(direction)
    email_display = email if email not in {"—", ""} else "官网未公开，请以教师主页为准"
    ...
    match, evidence, plan = customized_paragraphs(direction)
    body = re.sub(
...
if "<th>套磁信</th>" not in text:
    text = text.replace(
        '<th data-page-node-id="279gW2Zz3xjHeODBEnXTqn">主要研究方向</th>',
        '<th data-page-node-id="279gW2Zz3xjHeODBEnXTqn">主要研究方向</th><th>套磁信</th>',
    )
if ".letter-cell" not in text:
    text = text.replace(
        " .nm-note{color:#9aa3af;font-size:11px}\n",
        " .nm-note{color:#9aa3af;font-size:11px}\n"
