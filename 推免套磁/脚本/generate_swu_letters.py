...
HTML = ROOT / "推免套磁/西南大学/西南大学计算机学院教师名录及研究方向.html"
TEMPLATE = ROOT / "推免套磁/套磁信-大模型工程方向-修改版.md"
OUT = ROOT / "推免套磁/西南大学/个性化套磁信"
...
class FacultyParser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        if tag == "td" and self.in_td:
            ...
            if len(self.row) >= 9 and self.row[0].isdigit():
                self.rows.append(self.row[:9] + [self.href])
            self.in_tr = False
...
def clean_direction(direction):
    ...
    if direction in {"", "—", "未公开"}:
        return "人工智能相关研究"
    return re.sub(r"\s+", " ", direction)


def paragraphs(direction):
    original = direction
    d = clean_direction(direction)
    if d == "人工智能相关研究" and original in {"", "—", "未公开"}:
        return (
    ...
    if any(k in d for k in ("Agent", "AGENT", "智能软件", "智能体", "人工智能+")):
        return (
            ...
            f"若有机会加入您的课题组，我希望围绕{d}深入学习大模型推理、工具调用、"
            "多智能体协作与 Agent 评测，将已有的工程实践提升为规范的科研能力。",
        )
    if any(k in d for k in ("视觉", "图像", "图形", "模式识别", "文档识别", "图像分类", "视频")):
        return (
            ...
            f"若有机会加入您的课题组，我希望围绕{d}继续夯实视觉表征、检测、分割或多模态融合基础，"
            "学习严谨的实验设计和模型评价方法。",
        )
    if any(k in d for k in ("安全", "密码", "隐私", "攻防", "漏洞", "可信")):
        return (
    ...
    if any(k in d for k in ("机器学习", "深度学习", "数据挖掘", "模式识别", "优化", "人工智能", "预测")):
        return (
    ...
    if any(k in d for k in ("通信", "网络", "云计算", "物联网", "分布式", "嵌入式", "机器人")):
        return (
            f"您主要从事{d}等方面的研究，与我对实时智能系统、AI 工程落地和受限环境下智能交互的兴趣有较好交叉。",
            "我主导过面向军工场景的 CamTalk 多模态实时视觉对话 Agent，设计端侧数据最小化和关键帧智能采样，"
            "参与 STT–LLM–TTS 7 节点流式编排，并在复杂网络环境下积累了数据加载和接口兜底经验。",
            f"若有机会加入您的课题组，我希望围绕{d}补足系统与网络基础，探索大模型 Agent、边缘智能和实时计算的结合。",
        )
    if any(k in d for k in ("软件工程", "软件缺陷", "软件测试", "服务计算", "人机交互")):
        return (
...
def make_letter(row):
    index, name_cell, title, mentor, age, department, direction, email, intro, homepage = row
...
if "<th>套磁信</th>" not in text:
    text = text.replace(
        '<th data-page-node-id="FKy9uLXITq7hC5yFsabkPb">个人简介（悬停看全文）</th>',
        '<th data-page-node-id="FKy9uLXITq7hC5yFsabkPb">个人简介（悬停看全文）</th><th>套磁信</th>',
    )
...
def add_link(match):
    ...
    index_match = re.search(r'<td class="c-idx"[^>]*>\s*(\d+)\s*</td>', row_html)
